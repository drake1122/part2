...
// 접근통제 기능이 켜져 있다면 아이피 체크
if ($config['cf_use_admlogin'] && !empty($config['cf_admlogin_ip'])) {
    // 관리자 계정인지 체크
    $manager = sql_fetch("select mb_id from {$g5['eyoom_manager']} where mb_id = '{$mb_id}'");

    if ($mb_id == $config['cf_admin'] || is_admin($mb_id) || $manager) {
        alert('관리자는 일반 로그인 방식을 사용하실 수 없습니다.', G5_BBS_URL.'/admlogin.php');
    }
}
