...
class theme extends qfile
{
    ...
    protected function check_theme() {
        ...
        if (isset($_GET['theme']) || isset($_GET['shop_theme'])) {
            $_user['theme']      = clean_xss_tags(trim(isset($_GET['theme']) ? $_GET['theme'] : ''));
            $_user['shop_theme'] = clean_xss_tags(trim(isset($_GET['shop_theme']) ? $_GET['shop_theme'] : ''));
    ...
    private function get_sca_from_link($link) {
        ...
        $query_string = isset($str['query']) ? $str['query'] : '';
        if ($query_string) {
            parse_str($query_string, $query);
            return isset($query['sca']) ? $query['sca'] : '';
        }
        return '';
    }
    ...
    private function eyoom_subpage_info($theme) {
        ...
        foreach ($info as &$value) {
            $value = $value ? htmlspecialchars(trim($value), ENT_QUOTES, 'UTF-8') : '';
        }
    ...
    private function get_default_page() {
        ...
        switch($key) {
            ...
            case 'admlogin'         : $title = '관리자 로그인'; $cate_name = '관리자'; break;
