...
class bbs extends eyoom
{
    ...
    private function get_video_use_curl($url, $host) {
        ...
        switch($host) {
             ...
             */
            case 'vimeo.com':
                preg_match('/meta property="og:url"\s*content="([^"]+)"/i', $output, $matches);
                $out['vid'] = $this->get_video_key($this->eyoom_host($matches[1]));
                preg_match('/meta property="og:image"\s*content="([^"]+)"/i', $output, $matches);
                $out['imgsrc'] = $matches[1];
                return $out;
                break;
            ...
            case 'tvcast.naver.com':
            case 'tv.naver.com':
                // <meta property="og:url" content="https://tv.naver.com/v/94010313"/>
                preg_match('/meta property="og:url"\s*content="([^"]+)"/i', $output, $matches);
                $out['vid'] = $this->get_video_key($this->eyoom_host($matches[1]));
                preg_match('/meta property="og:image"\s*content="([^"]+)"/i', $output, $matches);
                $out['imgsrc'] = $matches[1];
    ...
    private function video_source($video) {
        switch($video['host']) {
            ...
            case 'tvpot.daum.net':
            case 'tv.kakao.com':
                $source = '<iframe width="'.$video['width'].'" height="'.$video['height'].'" src="https://play-tv.kakao.com/embed/player/cliplink/'.$video['key1'].'?service=player_share" frameborder="0" scrolling="no"></iframe>';
                break;
            case 'dailymotion.com':
            case 'dai.ly':
    ...
    public function get_imgurl_from_video($src) {
        ...
        // 모든 이미지 URL에서 쿼리 파라미터 제거 (? 이후 텍스트)
        $video['img_url'] = preg_replace('/\?.*$/', '', $video['img_url']);
    ...
    public function save_url_image($url, $local_image) {
        ...
        // 에러 확인
        if ($error || $httpCode != 200 || empty($rawdata)) {
            error_log('Image download failed: ' . $error . ' (HTTP ' . $httpCode . ')');
            return false;
        }
        ...
        // 파일 쓰기 (w 모드로 변경 - 안전성 향상)
        if ($fp = fopen($local_image, 'w')) {
            fwrite($fp, $rawdata);
            fclose($fp);
            return true;
        } else {
            error_log('Failed to create file: ' . $local_image);
            return false;
        }
    }
